...
一个**单文件离线可用**的 BTC K 线复盘网页。所有数据（1d / 4h / 1h / 15 分，15m 全量 24.4 万根）全部内联在一个 `index.html` 中，双击即可离线打开，也可直接部署到 GitHub Pages 分享链接。
...
BTC-USDT 永续合约 15 分钟 K 线，来自 [k--data 数据仓库](https://github.com/sdlyingyong/k--data)（币安 BTCUSDT 永续），1h/4h/1d 由 15m 重采样生成。数据更新方式：

```
node sync_from_kdata.cjs        # 同步 k--data 最新数据（默认读取 ../k--data，--repo= 指定路径，--remote 自动拉取）
```
